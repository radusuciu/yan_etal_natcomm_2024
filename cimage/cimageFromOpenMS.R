...
source(paste(cimage.path, "/inputparams.R", sep=""))
source(paste(cimage.path, "/msisotope.R", sep=""))
...
# find all matched mzML input files in upper directory
input.path <- getwd()
mzML.names <- list.files(path="../", pattern="mzML$")
mzML.files <- as.list( mzML.names )
names(mzML.files) <- mzML.names
for (name in mzML.names) {
  # cat(paste(name, "\n", sep=""))
  mzML.files[[name]] <- xcmsRaw( paste("../", name, sep=""), profstep=0, includeMSn=T)
}
...
# dir.create(paste(output.path, "/PNG", sep="")) # ken deprecate
npages <- dim(cross.table)[1]
cat(paste("Cimage: total number of pages are ", npages, sep=""))
for ( i in 1:npages) {
  ...
  # do not know what it is for <-gabe?
  # appears to run through the possible mzML to merge different ms runs/aka fractions/aka steps
  # cat(exist.index)
  for ( k in 1:length(exist.index) ) {
    kk <- exist.index[k]
	  # cat(kk)
    raw.file <- paste( cross.vec[kk], "_", segment, ".mzML", sep="")
    # message(paste("Found mzML file ", raw.file, sep=""))
    xfile <- mzML.files[[raw.file]]
